...
public interface MessageRepository extends JpaRepository<MessageModel, Long> {
  @Query(value = "SELECT m FROM MessageModel m WHERE (m.sender.id = ?1 AND m.receiver.id = ?2) OR (m.sender.id = ?2 AND m.receiver.id = ?1) AND (m.time >= ?3) ORDER BY m.time")
  List<IMessageModel> findConversationQuery(Long firstUserId,
                                            Long secondUserId, 
                                            Timestamp dateTimeFrom);
